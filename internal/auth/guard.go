...
func WithLogger(log *slog.Logger) GuardOption {
	return func(g *guard) {
		if log != nil {
			g.log = log
		}
...
func (g *guard) Permits(w http.ResponseWriter, r *http.Request) bool {
	log := g.log.With("method", r.Method, "path", r.URL.Path)
	...
	if err != nil {
		log.Error("Unexpected error bouncing request", "error", err)
		w.WriteHeader(http.StatusInternalServerError)
		return false
	}

	if err := g.stamper.Stamp(r, access); err != nil {
		log.Error("Unexpected error stamping request", "error", err)
		w.WriteHeader(http.StatusInternalServerError)
		return false
	}

	log.Debug("Access granted", "user", access.User, "roles", access.Roles)
	return true
}
